...
class TestNode6(Node):
    def __init__(self):
        ...
        self.after_pick_waypoint_joints = [0.0, 6.0, -17.0, 64.0]
...
#########################
    ...
    def run_command_sequence(self, cmd: int):
        ...
        try:
            while rclpy.ok():
                ...
                self.get_logger().info(f'{name} class 확인됨. 좌표 안정화를 위해 대기')
    ...
    def execute_enhanced_sequence(self, cmd: int, target: Dict) -> bool:
        ...
        if raw_tx <= self.near_x_threshold_m:
            ...
            self.send_arm_joint_topic(self.after_pick_waypoint_joints, self.fast_transfer_motion_sec)
            self.wait_joint_motion('pick 후 경유지 이동', self.fast_transfer_motion_sec)

        self.send_arm_joint_topic(self.place_hover_joints[cmd], self.fast_transfer_motion_sec)
        self.wait_joint_motion('분류 상자 이동 성공', self.fast_transfer_motion_sec)
