...
class TestNode4(Node):
    def __init__(self):
        super().__init__('test_node4')
        ...
        self.get_logger().info('START test_node4: test_node3 + test_sequence 동작 결합')
    ...
    def run_command_sequence(self, cmd: int):
        ...
        try:
            while rclpy.ok():
                ...
                if not ok:
                    self.get_logger().warn('시퀀스 실패: 해당 타겟 스킵하고 다음 타겟 탐색 계속')
                    continue
                count += 1
        finally:
            self.send_arm_joint_topic(self.hand_recognition_joint_deg)
