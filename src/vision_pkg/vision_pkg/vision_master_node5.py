            ...
            if results.boxes is not None:
                for box in results.boxes:
                    ...
                    if 0.1 < z_val < 1.0: 
                        ...
                        self.CAMERA_TO_BASE_X = 0.023
                        
                        # 🌟 [좌표계 변환 핵심] 카메라 축을 로봇 베이스 축으로 매핑 (예시)
                        # 카메라 Z(깊이) -> 로봇 X(앞뒤)
                        # 카메라 X(가로) -> 로봇 -Y(좌우)
                        # 카메라가 측정한 물체까지의 거리(p[2]) + 카메라가 이미 앞에 나가있는 거리
                        robot_x = point_3d[2] + self.CAMERA_TO_BASE_X 
                        robot_y = -point_3d[0] # 좌우
                        robot_z = 0.04  # 높이는 고정하기로 했죠?
